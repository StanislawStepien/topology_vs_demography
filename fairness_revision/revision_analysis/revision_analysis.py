...
def cmd_stats():
    ...
    for i, t in enumerate(TOPICS, 1):
        ...
        dummy_f1 = bin_f1(y, dmf)
        print("### %s  (n_test=%d, P(mispred)=%.2f, dummy-most-freq F1=%.3f)" % (t, n, bal, dummy_f1))
        for lab, p in [("demography", pD), ("topology", pT), ("hybrid", pH)]:
            f1 = bin_f1(y, p);
            lo, hi = boot_ci(y, p)
            pr = precision_score(y, p, pos_label=1, zero_division=0);
            rc = recall_score(y, p, pos_label=1, zero_division=0)
    ...
    with open(OUT_DIR / "stats" / "general_pop_metrics.csv", "w", newline="") as f:
        w = _csv.DictWriter(f, fieldnames=["question", "approach", "f1", "ci_low", "ci_high", "precision", "recall",
                                           "dummy_f1", "balance", "n_test"])
        w.writeheader();
        w.writerows(metrics)
...
def cmd_minorities():
    ...
    for t in TOPICS:
        ...
        for cat, df in mino.items():
            ...
            mask = Xte.index.isin(idx);
            n_sub = int(mask.sum())
            if n_sub == 0: continue
            y_sub = y[mask];
            n_pos = int(np.sum(y_sub == 1))
            for lab in ("demography", "topology", "hybrid"):
                p_sub = np.asarray(preds[lab])[mask];
                lo, hi = boot_ci(y_sub, p_sub)
            ...
            hp = np.asarray(preds["hybrid"])[mask];
            lo, hi = boot_ci(y_sub, hp)
            print(
                "   %-24s n=%-3d pos=%-3d  hybrid F1=%.3f [%.2f,%.2f]" % (cat, n_sub, n_pos, bin_f1(y_sub, hp), lo, hi))
    outp = OUT_DIR / "stats" / "per_minority_ci.csv";
    outp.parent.mkdir(parents=True, exist_ok=True)
    with open(outp, "w", newline="") as f:
        w = _csv.DictWriter(f, fieldnames=["question", "minority", "approach", "n_test", "n_positive", "f1", "ci_low",
                                           "ci_high"])
        w.writeheader();
        w.writerows(rows)
...
if __name__ == "__main__":
    ...
    ap.add_argument("cmd", choices=["rebuild", "stats", "shap", "minorities", "all"])
    c = ap.parse_args().cmd
