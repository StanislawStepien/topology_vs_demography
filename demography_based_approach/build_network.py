...
def draw_graph_of_g(G, show):
    ...
    nx.draw_networkx(G, pos=nx.spring_layout(G, 1), with_labels=True, edge_color=edges_colormap)
    if show:
        plt.show()


def draw_adjacency_heatmap(adjacency_weights):
    sns.heatmap(adjacency_weights, annot=True, cmap=sns.color_palette("Spectral", as_cmap=True))
    plt.show()
